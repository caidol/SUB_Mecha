...
async def configure_poll(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int: # start poll config
    LOGGER.info("Polls Module: Poll configuration started.")
    ...
    try:
        ...
        LOGGER.info("Polls Module: Poll question request sent to chat ")
    except:
        LOGGER.error("Polls Module: Poll question request unable to be sent to chat.")
...
async def poll_question(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int: # receive poll question
    ...
    LOGGER.info("Polls Module: Poll question retrieved from update object.")

    # store question as key
    LOGGER.info("Polls Module: Question has been stored as a key in user data.")
    ...
    try:
        ...
        LOGGER.info("Polls Module: Poll responses request sent to chat.")
    except: 
        LOGGER.error("Polls Module: Poll responses request was unable to be sent to chat.")
...
async def poll_answers(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int: # receive poll answers
    ...
    LOGGER.info("Polls Module: Poll responses retrieved from update object.")

    # store responses as values
    LOGGER.info("Polls Module: Poll question and responses are being stored in user data.")
    ...
    for i in context.user_data.keys(): 
        if i == poll_question:
            LOGGER.info("Polls Module: Storing the poll question and separated values in user data.")
    ...
    try:
        ...
        LOGGER.info("Polls Module: Poll completion message has been sent to chat.")
    except:
        LOGGER.error("Polls Module: Poll completion message was unable to be sent to chat.")
...
async def configure_quiz(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int: # start quiz config
    if str(update.effective_chat.id)[0] == '-':
        ...
        try:
            ...
            LOGGER.info("Polls Module: Quiz redirect message has been sent to chat.") 
        except:
            LOGGER.error("Polls Module: Quiz redirect message was unable to be sent to chat.")
        ...
        try:
            ...
            LOGGER.info("Polls Module: Quiz question request has been sent to chat.")
        except:
            LOGGER.error("Polls Module: Quiz question request was unable to be sent to chat.")
...
async def quiz_question(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int: # receive quiz question
    ...
    LOGGER.info("Polls Module: Quiz question retrieved from update object.")    
    ...
    try:
        ...
        LOGGER.info("Polls Module: Quiz answers request has been sent to chat.")
    except:
        LOGGER.error("Polls Module: Quiz answers request was unable to be sent to chat.")
...
async def quiz_answers(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int: # receive quiz answers
    ...
    LOGGER.info("Polls Module: Quiz answers retrieved from update object.")

    context.user_data[quiz_question] = quiz_answers

    for i in context.user_data.keys():
        if i == quiz_question:
            LOGGER.info("Polls Module: Storing the quiz question and separated answers in user data.")
            regex_expression = re.compile(r"[, ]\s?")
            global delimiter
    ...
    try:
        ...
        LOGGER.info("Polls Module: Quiz correct answer message has been sent to chat.")
    except:
        LOGGER.error("Polls Module: Quiz correct answer message was unable to be sent to chat.")
...
async def correct_quiz_answer(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int: # receive correct quiz answer
    ...
    LOGGER.info("Polls Module: Correct answer retrieved from update message object.")

    global answer_dict
    LOGGER.info("Polls Module: Correct answer stored in the answer dictionary.")
    ...
    try:
        ...
        LOGGER.info("Polls Module: Quiz completion message has been sent to chat.")
    except:
        LOGGER.error("Polls Module: Quiz completion message was unable to be sent to chat.")
...
async def cancel_poll(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int: # cancel command
    ...
    LOGGER.info("Polls Module: User %s canceled the conversation.", user.first_name)
    ...
    try:
        ...
        LOGGER.info("Polls Module: Cancel poll message was sent to chat.")
    except:
        LOGGER.error("Polls Module: Cancel poll message was unable to be sent to chat.")
...
async def poll(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    for key, value in context.user_data.items():
        if key == poll_question and value == poll_answers.split(delimiter.group(0)):
            LOGGER.info("Polls Module: Poll question and poll answer located in user_data.")
            question = key
            responses = value
    ...
    try:
        ...
        LOGGER.info("Polls Module: Poll was able to be sent to chat.")
    except:
        LOGGER.error("Polls Module: Poll was unable to be sent to chat.")
    ...
    LOGGER.info("Polls Module: Poll payload information stored in bot_data")
...
async def receive_poll_answer(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    LOGGER.info("Polls Module: The poll answer was retreived from the update object.")
    answered_poll = context.bot_data[answer.poll_id] # get the bot data relating to the specific poll id key
    try:
        LOGGER.info("Polls Module: Retrieving the poll via the questions.")
        questions = answered_poll["questions"]
    # otherwise the poll answer update is from an old poll, so we are unable to answer in that case
    except KeyError:
        LOGGER.error("Polls Module: Unable to retrieve te poll question because of a KeyError.")
        return
    ...
    try:
        ...
        LOGGER.info("Polls Module: The poll answer user was sent to chat.")
    except:
        LOGGER.error("Polls Module: The poll answer user was unable to be sent to chat.")
    ...
    if answered_poll["answers"] == TOTAL_VOTER_COUNT:
        LOGGER.warning("Polls Module: The poll answers has reached the maximum voter limit.")
...
async def receive_poll(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    LOGGER.info("Polls Module: The poll message was retrieved from the update object.")
    ...
    try:
        ...
        LOGGER.info("Polls Module: The closed poll was sent to chat.")
    except:
        LOGGER.error("Polls Module: The closed poll was unable to be sent to chat.")
...
async def quiz(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None: 
    ...
    for key, value in context.user_data.items():
        if key == quiz_question and value == quiz_answers.split(delimiter.group(0)):
            LOGGER.info("Polls Module: Quiz question and quiz answers located in user_data.")
            question = key
            answers = value

    for key, value in answer_dict.items():
        if key == quiz_question:
            LOGGER.info("Polls Module: Quiz question has been located for correct answer.")
            correct_answer = value

    for i in range(len(answers)):
        if correct_answer == answers[i]:
            LOGGER.info("Polls Module: Correct id for quiz answer has been located.")
            correct_id = i
    ...
    LOGGER.info("Polls Module: Quiz payload information stored in bot_data.")
...
async def receive_quiz_answer(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if update.poll.total_voter_count == TOTAL_VOTER_COUNT:
        LOGGER.warning("Polls Module: The quiz answers has reached the maximum voter limit.")
        try:
            quiz_data = context.bot_data[update.poll.id]
            LOGGER.info("Polls Module: Quiz payload information has been retrieved from bot data given the id.")
            # this means this poll answer update is from an old poll, we can't stop it then
        except KeyError:
            LOGGER.error("Polls Module: Unable to retrieve quiz payload information given id.")
            return
        
        try:
            await context.bot.stop_poll(quiz_data["chat_id"], quiz_data["message_id"])
            LOGGER.info("Polls Module: Quiz was able to be stopped.")
        except:
            LOGGER.error("Polls Module: Quiz was unable to be stopped.")
...
async def preview(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        LOGGER.info("Polls Module: Poll preview was sent to chat.")
    except:
        LOGGER.error("Polls Module: Poll preview wsa unable to be sent to chat.")


def main():
    # Add a conversation handler with multiple states
    LOGGER.info("Polls Module: Creating and adding handlers")
